...
describe('release workspace classification', () => {
  it('keeps the desktop deployment private while publishing ordinary apps', () => {
    expect(isReleaseMemberDirectory('apps/desktop')).toBe(false)
    expect(isReleaseMemberDirectory('apps/cli')).toBe(true)
    expect(isReleaseMemberDirectory('apps/web')).toBe(true)
  })
})
