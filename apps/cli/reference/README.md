...
The app's DSH and Cordis packages must come from the current installation. Before pnpm runs, the command records the profile dependency files and existing runtime-package copies; after a successful pnpm operation, any newly introduced profile-local copy of an app-owned runtime package makes the command fail. The command restores the recorded files, asks pnpm to materialize the previous dependency graph without lifecycle scripts, and verifies that the new copies are gone before reporting a successful rollback. A rollback that cannot restore the graph leaves the profile startup-blocked with its directory in the diagnostic; every profile launch independently rejects any remaining runtime-package copy. Plugin packages therefore declare DSH and Cordis packages as `peerDependencies`, relying on the installation fallback rather than bundling another Harness runtime.

This check prevents dependency shadowing; it does not make third-party code trustworthy. An enabled plugin can intentionally add or replace UI behavior, and any lifecycle script explicitly allowed through pnpm executes with the user's permissions.
