...
afterEach(() => {
  ...
  document.head.querySelectorAll('meta[name^="dsh-boot-logo"]').forEach((node) => { node.remove() })
})
...
describe('BootPage', () => {
  ...
  it('uses application metadata for a branded boot logo', () => {
    document.head.insertAdjacentHTML('beforeend', [
      '<meta name="dsh-boot-logo" content="/brand.png">',
      '<meta name="dsh-boot-logo-alt" content="Product brand">',
    ].join(''))
    const { el } = mount()
    const image = el.querySelector<HTMLImageElement>('img')
    expect(image?.getAttribute('src')).toBe('/brand.png')
    expect(image?.alt).toBe('Product brand')
