...
/** Read a non-empty application metadata value. */
...
/** Build the application-supplied boot logo, or the generic local fallback. */
function bootWordmark(): HTMLElement {
  const source = metadata('dsh-boot-logo')
  if (source === undefined) return div(css.wordmark, 'HARNESS')
  const image = document.createElement('img')
  image.className = css.wordmarkImage ?? ''
  image.src = source
  image.alt = metadata('dsh-boot-logo-alt') ?? ''
  return image
}
